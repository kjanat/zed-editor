...
To turn `format-on-save` on, you can enable the setting globally or for
...
Many JavaScript projects use command-line code-formatting tools, such as
[Prettier](https://prettier.io/). You can use one of these tools by specifying
an *external* code formatter for JavaScript in your settings. See
[the configuration docs](../reference/all-settings.md) for more information.
...
JavaScript, or add to your settings file:
...
Zed supports JSDoc syntax in JavaScript and TypeScript comments that match the
JSDoc syntax. Zed uses
...
Configure code actions on format in Settings ({#kb zed::OpenSettings}) under
Languages > JavaScript, or add to your settings file:
...
> ESLint. So if your language server or Prettier configuration doesn't format
...
Configure in Settings ({#kb zed::OpenSettings}) under Languages > JavaScript, or
...
in vanilla JavaScript files (`.js`), you can customize the `classRegex` field
