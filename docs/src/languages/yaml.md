...
by adding them to your Zed settings.json in a `yaml-language-server` block under
the `lsp` key.

You can configure custom YAML schemas using relative paths. Zed resolves paths
...
To use `yaml-language-server` instead of Prettier for YAML formatting, configure
in Settings ({#kb zed::OpenSettings}) under Languages > YAML, or add to your
settings file:
