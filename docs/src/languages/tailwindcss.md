...
Zed has built-in support for Tailwind CSS autocomplete, linting, and hover
previews.
...
If by default the language server isn't enough to make Tailwind work for a given
language, you can configure the language server settings and add them to the
`lsp` section of your `settings.json`:
...
Refer to
[the Tailwind CSS language server settings docs](https://github.com/tailwindlabs/tailwindcss-intellisense?tab=readme-ov-file#extension-settings)
...
`@layer`, and `@theme`. Configure language servers in Settings ({#kb
zed::OpenSettings}) under Languages > CSS, or add to your settings file:
...
Zed supports Prettier out of the box, which means that if you have the
