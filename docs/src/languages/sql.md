...
SQL files are handled by the
[SQL Extension](https://github.com/zed-extensions/sql).

- Tree-sitter:
  [nervenes/tree-sitter-sql](https://github.com/nervenes/tree-sitter-sql)

### Formatting

Zed supports auto-formatting SQL using external tools like
...
3. Configure formatting in Settings ({#kb zed::OpenSettings}) under Languages >
   SQL, or add to your settings file:
...
You can add this to Zed project settings (`.zed/settings.json`) or via your Zed
...
When using a `.sql-formatter.json` file you can use a simplified Zed settings
configuration:
