...
When inlay hints are enabled in Zed, to make the language server send them back,
Zed sets the following initialization options:
...
in Svelte files, you need to configure the language server so that it knows
...
With these settings, you will get completions for Tailwind CSS classes in Svelte
files. Examples:
