...
The snippets are located in `~/.config/zed/snippets` directory to which you can
navigate with the {#action snippets::OpenFolder} action.
...
Path separators (`/` and `\`) are removed from the file name, so a language
named `PL/X` uses `plx.json`.
