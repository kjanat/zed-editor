...
In addition to the modal outline (`cmd-shift-o`), Zed offers an outline panel.
...
the current buffer's symbols. Each symbol entry shows its type prefix (such as
"struct", "fn", "mod", "impl") along with the symbol name, helping you quickly
identify what kind of symbol you're looking at. Clicking on an entry allows you
to jump to the associated section in the file. The outline view will also
...
The outline panel truly excels when used with multi-buffers. Here are some
...
The outline view provides a great way to quickly navigate to specific parts of
your code and helps you maintain context when working with large result sets in
multi-buffers.
