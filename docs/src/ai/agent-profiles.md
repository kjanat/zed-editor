...
A profile can set a default model and choose which [built-in tools](./tools.md)
...
Open the profile selector in the [Agent Panel](./agent-panel.md), then click
`Configure`.
...
Agent profiles apply to the Zed Agent. External Agents and
[Terminal Threads](./terminal-threads.md) do not use Zed Agent profiles unless
