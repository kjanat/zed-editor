...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Analyze highlights.scm files for unique instances and their languages."
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
        help="Include a list of languages for each tag.",
    )
...
def count_instances(files):
    instances: defaultdict[list[Any], dict[str, Any]] = defaultdict(
        lambda: {"count": 0, "languages": set()}
    )
...
def main():
    ...
    unique_extension_instances = {
        k: v for k, v in extension_instances.items() if k not in core_instances
    }
