...
fn watch_needs_rearm(kind: OsWatcherKind) -> bool {
    kind == OsWatcherKind::Native && !cfg!(target_os = "macos")
}
...
impl WatchRearm {
    ...
    fn rearm(&self) {
        ...
        registration.os_watcher.remove(registration.id);
        let (Some(fs), Some(native_watcher), Some(poll_watcher)) = (
            self.fs.upgrade(),
            self.native_watcher.upgrade(),
            self.poll_watcher.upgrade(),
        ) else {
            return;
        };
...
impl OsWatcher {
    ...
    pub(crate) fn with_backend(
        ...
        Arc::new(Self {
            kind,
            recursive: AtomicBool::new(kind.is_recursive()),
            state,
    ...
    pub(crate) fn set_non_recursive(&self) {
    ...
    fn add(
        ...
        let path_already_covered = state
            .paths
            .covered_by_recursive_ancestor(&path, self.is_recursive());
...
mod tests {
    ...
    fn pending_watch_yields_during_watch_limit_cooldown(cx: &mut gpui::TestAppContext) {
        ...
        let native_watcher = Arc::new(test_os_watcher(
            OsWatcherKind::Native,
            Some(backend.clone()),
        ));
        ...
        watcher
            .add(&path)
            .expect("defer watch after limit is reached");
        cx.executor().run_until_parked();
        assert_eq!(backend.lock().watch_calls, std::slice::from_ref(&path));
        assert!(
            watcher
                .pending_registrations
                .lock()
        ...
        native_watcher.state.lock().cooldown_until = None;
        cx.executor().advance_clock(poll_interval());
        cx.executor().run_until_parked();
        assert!(watcher.pending_registrations.lock().is_empty());
        assert_eq!(backend.lock().watch_calls, [path.clone(), path]);
